...
def main():
    ...
    classifiers = create_classifiers(input_size_elm = X_train.shape[1])
    #class_name = 'elm'
    for class_name in classifiers.keys():
        classifier = classifiers.get(class_name)
        time_for_training = fit_classifier(classifier, train_features, y_train)
        classifier_accuracy = classifier.score(test_features, y_test)
        print("Accuracy of",class_name,":{:.3f}",classifier_accuracy, "\t training took:{:.5f}[s]", time_for_training)
